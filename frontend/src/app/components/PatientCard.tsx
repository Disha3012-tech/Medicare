import { AlertCircle, CheckCircle2, Clock, ChevronRight } from "lucide-react";
import type { PatientSummary } from "../data/patients";

const RISK_CONFIG = {
  low:    { label: "Low risk",    className: "bg-accent/10 text-accent" },
  medium: { label: "Medium risk", className: "bg-yellow-50 dark:bg-yellow-950/20 text-yellow-600 dark:text-yellow-400" },
  high:   { label: "High risk",   className: "bg-destructive/10 text-destructive" },
};

interface Props { patient: PatientSummary; onClick: () => void; }

export default function PatientCard({ patient: p, onClick }: Props) {
  const risk = RISK_CONFIG[p.risk];
  return (
    <div onClick={onClick} className="bg-card rounded-xl border border-border p-4 flex gap-3 hover:shadow-sm hover:border-accent/20 cursor-pointer transition-all group">
      <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-sm font-semibold text-primary flex-shrink-0">
        {p.name.split(" ").map(n => n[0]).join("").slice(0, 2)}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <p className="font-medium text-foreground text-sm truncate">{p.name}</p>
          <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${risk.className}`}>{p.risk}</span>
        </div>
        <p className="text-xs text-muted-foreground mt-0.5">{p.age} yrs · {p.gender} · {p.bloodGroup}</p>
        <p className="text-xs text-muted-foreground mt-1 truncate">{p.condition}</p>
        <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
          <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> Last: {p.lastVisit}</span>
          {p.nextVisit && <span className="flex items-center gap-1 text-accent"><CheckCircle2 className="w-3 h-3" /> Next: {p.nextVisit}</span>}
        </div>
      </div>
      <ChevronRight className="w-4 h-4 text-muted-foreground group-hover:text-foreground transition-colors flex-shrink-0 mt-1" />
    </div>
  );
}
