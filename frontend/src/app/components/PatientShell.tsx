...
import {
  HeartPulse, Calendar, FileText, Pill, Stethoscope, Bell,
  Settings, LogOut, LayoutDashboard, Activity, MessageCircle,
} from "lucide-react";
...
const NAV = [
  ...
  { path: "/patient/history",       label: "Appointments",     icon: Calendar },
...
export default function PatientShell({ title, subtitle, children, actions }: Props) {
    ...
    <div className="min-h-screen bg-background flex font-['Inter',sans-serif]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-card border-r border-border flex flex-col transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        ...
        <div className="p-3 border-t border-border space-y-0.5">
          <button onClick={() => { navigate("/patient/notifications"); setOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${isActive("/patient/notifications") ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>
