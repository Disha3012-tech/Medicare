      ...
      <div className="p-5 flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{formatTime(appt.scheduled_at)}</span>
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t border-border pt-4 space-y-2">
          <div>
            ...
            <p className="text-sm text-foreground">{appt.reason_for_visit || "Not specified"}</p>
          </div>
          {appt.notes && (
            <div>
          ...
          {appt.cancel_reason && (
            <div>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">{appt.cancel_reason}</p>
            </div>
          )}
          <div className="text-xs text-muted-foreground">
            <span>Duration: <span className="font-medium text-foreground">{appt.duration_min} min</span></span>
          </div>
        </div>
      )}
    </div>
